            ...
            <div x-data="{ open: false }" class="flex flex-col max-w-screen-xl p-5 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-8">
                ...
                <nav :class="{'flex': open, 'hidden': !open}" class="flex-col flex-grow hidden md:flex md:justify-start md:flex-row">
                    <ul class="space-y-2 list-none lg:space-y-0 lg:items-center lg:inline-flex">
                        <li>
                            <a href="inicio.php" class="px-2 lg:px-6 py-6 text-sm border-b-2 border-transparent hover:border-blue-600 leading-[22px] md:px-3 text-gray-500 hover:text-blue-500 font-serif">Inicio</a>
                        </li>
                        <li>
                            <a href="reserva.php" class="px-2 lg:px-6 py-6 text-sm border-b-2 border-transparent leading-[22px] md:px-3 text-gray-500 hover:text-blue-500 hover:border-blue-600 font-serif">Reserva</a>
                        </li>
                        <li>
                            <a href="precios.php" class="px-2 lg:px-6 py-6 text-sm border-b-2 border-transparent hover:border-blue-600 leading-[22px] md:px-3 text-gray-500 hover:text-blue-500 font-serif">Precios</a>
                        </li>
                        <li>
                            <a href="contacto.php" class="px-2 lg:px-6 py-6 text-sm border-b-2 border-transparent leading-[22px] md:px-3 text-gray-500 hover:text-blue-500 hover:border-blue-600 font-serif">Contacto</a>
                        </li>
                        <li>
                            <a href="desconectar.php" class="px-2 lg:px-6 py-6 text-sm border-b-2 border-transparent leading-[22px] md:px-3 text-gray-500 hover:text-blue-500 hover:border-blue-600 font-serif">Salir</a>
                        </li>
                    </ul>
                </nav>
            </div>
...
<div class="bg-white min-h-screen flex items-center">
    <div class="container mx-auto px-4">
        <div class="flex flex-wrap items-center justify-center">
            <div class="w-full md:w-6/12 lg:w-4/12 bg-white p-8 rounded shadow-md border border-blue-300"> <!-- Añadido el borde y ajuste -->
                <h3 class="text-2xl font-semibold mb-4 text-center">Reserva tu Espacio</h3>
                <form action="datos.php" method="POST" class="grid grid-cols-6 gap-6">
                    <div class="col-span-6">
                        <label for="fullname" class="block text-sm font-medium text-gray-700">Nombre</label>
                        <input type="text" name="nombre" id="fullname" class="mt-1 w-full rounded-lg border border-blue-300 bg-white text-sm text-gray-700 shadow-sm px-3 py-2" required> <!-- Ajuste del tamaño del input y agregado borde -->
                    </div>
                    <div class="col-span-6">
                        <label for="activities" class="block text-sm font-medium text-gray-700">Ocupacion</label>
                        <select name="ocupacion" id="activities" class="mt-1 w-full rounded-lg border border-blue-300 bg-white text-sm text-gray-700 shadow-sm px-3 py-2" required> <!-- Ajuste del tamaño del select y agregado borde -->
                            <option value="Ocupacion">Freelance</option>
                            <option value="Estudiante">Estudiante</option>
                            <option value="Excursionista">Empleado</option>
                        </select>
                    </div>
                    <div class="col-span-6">
                        <label for="destination" class="block text-sm font-medium text-gray-700">Oficinas</label>
                        <select name="destino" id="destination" class="mt-1 w-full rounded-lg border border-blue-300 bg-white text-sm text-gray-700 shadow-sm px-3 py-2" required> <!-- Ajuste del tamaño del select y agregado borde -->
                            <option value="Granada Centro">Granada Centro</option>
                            <option value="Granada Sur">Granada Sur</option>
                            <option value="Granada Norte">Granada Norte</option>
                        </select>
                    </div>
                    <div class="col-span-6">
                        <label for="precio" class="block text-sm font-medium text-gray-700">Tipo de Membresia</label>
                        <select name="precio" id="precio" class="mt-1 w-full rounded-lg border border-blue-300 bg-white text-sm text-gray-700 shadow-sm px-3 py-2" required> <!-- Ajuste del tamaño del select y agregado borde -->
                            <option value="€ 50">€ 50 basico</option>
                            <option value="€ 120">€ 120 premium</option>
                            <option value="€ 150 ">€ 150 Vip</option>
                        </select>
                    </div>
                    <div class="col-span-6">
                        <label for="date-start" class="block text-sm font-medium text-gray-700">Fecha de reserva</label>
                        <input type="date" name="fecha" id="date-start" class="mt-1 w-full rounded-lg border border-blue-300 bg-white text-sm text-gray-700 shadow-sm px-3 py-2" required> <!-- Ajuste del tamaño del input y agregado borde -->
                    </div>
                    <div class="col-span-6">
                        <input type="submit" class="mt-4 w-full bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500" value="Enviar"> <!-- Ajuste del tamaño del botón -->
                    </div>
                </form>
            </div>
        </div>
